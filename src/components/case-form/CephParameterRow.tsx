...
export const CephParameterRow: React.FC<CephParameterRowProps> = ({
  label,
  norm,
  value,
  onChange,
  ...
  const isLocked = disabled || isAutoFetched;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap w-full">
        ...
        {isAutoFetched ? (
          <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-slate-700 bg-slate-200/90 border border-slate-300 px-2.5 py-0.5 rounded-full shrink-0 shadow-2xs">
            <Lock className="w-2.5 h-2.5 text-slate-500 shrink-0" />
            <span>Auto-fetched from {autoFetchedSource}</span>
          </span>
        ) : (
      ...
      <div className="flex items-center justify-between gap-3 pt-1 border-t border-slate-100 w-full">
        <div className="min-w-0 flex-1">
          ...
          {isMeasured ? (
            ...
            <div className="text-[11px] italic text-slate-400 mt-0.5">
              {isAutoFetched ? 'Auto-fetching value...' : 'Enter value'}
            </div>
          )}
        </div>
        ...
        <div className="w-24 sm:w-28 shrink-0">
          <input
            ...
            placeholder="0.0"
            className={`w-full min-h-[42px] px-3 py-1.5 rounded-xl border text-xs sm:text-sm font-extrabold text-right transition-all focus:outline-none focus:ring-2 ${
              isAutoFetched
                ? 'bg-slate-200/70 border-slate-300 text-slate-600 cursor-not-allowed font-mono opacity-80'
                : 'bg-white border-teal-400 text-slate-900 focus:ring-teal-500 focus:border-teal-500 shadow-2xs'
            }`}
          />
        </div>
      </div>
...
export default React.memo(CephParameterRow);
