        ...
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              onClick={() => {
                onUpdate(tooth, Number(opt));
                onClose();
              }}
              className={`w-full px-3 py-3 text-left text-sm font-semibold border-b border-slate-100 last:border-b-0 ${
                active ? 'bg-teal-50 text-teal-900' : 'text-slate-800 active:bg-slate-50'
              }`}
            >
              {opt} mm
            </button>
...
function ToothWidthArchSection({
  ...
  const renderCell = (tooth: string) => (
    <ToothWidthCell
      key={tooth}
      tooth={tooth}
      value={toothWidths[tooth] ?? ''}
      isActive={activeTooth === tooth}
      onOpen={() => setActiveTooth(tooth)}
    />
  );

  return (
    <div className="space-y-2">
      <span className="font-bold text-teal-900 text-xs block">{title}</span>
...
export const TabModelAnalysis: React.FC<TabModelAnalysisProps> = (props) => {
  ...
  const careyUpper = calculateCarey(props.toothWidths, props.maxillaryArchLengthAvailable);
  const ponts = calculatePonts(props.toothWidths);

  // Total Tooth Material for Ashley Howe (Max 12 or Carey sum from grid)
        ...
        <div className={STACK}>
          ...
          <div className={CARD}>
            <div className="flex flex-col gap-2 border-b border-slate-200 pb-2.5">
              <h4 className="font-bold text-slate-900 text-sm">1. Carey's Arch Perimeter Analysis</h4>
              {getBadgeComponent(careyUpper.badgeColor)}
            </div>

            <div className={STACK}>
              <NumericSelectField
                label="Arch Length Available (mm)"
                value={props.maxillaryArchLengthAvailable}
                onChange={props.setMaxillaryArchLengthAvailable}
                min={55}
                max={95}
                step={0.5}
                placeholder="Tap to select arch length…"
                presets={[{ label: 'typical maxillary', val: 72 }]}
              />

              <div>
                <label className="block text-slate-900 font-bold text-sm mb-1.5">Total Tooth Material (mm)</label>
                <div className="min-h-11 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-900 text-sm flex items-center">
                  {careyUpper.totalToothMaterial > 0
                    ? `${careyUpper.totalToothMaterial.toFixed(1)} mm`
                    : '—'}
                </div>
                <p className="text-xs text-slate-500 mt-1 leading-snug">
                  Auto-calculated from teeth 15–25 in the grid above
                </p>
              </div>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-xs sm:text-sm space-y-1">
              <div className="flex justify-between font-semibold text-slate-800">
                ...
                <span className={careyUpper.discrepancy !== null && careyUpper.discrepancy < 0 ? 'text-rose-600 font-bold' : 'text-slate-900 font-bold'}>
                  {careyUpper.discrepancy !== null ? `${careyUpper.discrepancy > 0 ? '+' : ''}${careyUpper.discrepancy.toFixed(1)} mm` : 'N/A'}
                </span>
              </div>
              <p className="font-bold text-teal-950 border-t border-slate-200 pt-1.5 mt-1">
                Inference: {careyUpper.inference}
              </p>
            </div>
          </div>
          ...
          <div className={CARD}>
            ...
            <div className={STACK}>
              <div>
                <label className="block text-slate-900 font-bold text-sm mb-1.5">Measured Premolar Width 4-4 (mm)</label>
                <div className="min-h-11 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-900 text-sm flex items-center">
                  {ponts.measuredPremolarWidth > 0 ? `${ponts.measuredPremolarWidth.toFixed(1)} mm` : '—'}
                </div>
                <p className="text-xs text-slate-500 mt-1 leading-snug">
                  Auto-calculated from teeth 14, 15, 24, 25 in the grid above
                </p>
              </div>

              <div>
                <label className="block text-slate-900 font-bold text-sm mb-1.5">Measured Molar Width 6-6 (mm)</label>
                <div className="min-h-11 px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-900 text-sm flex items-center">
                  {ponts.measuredMolarWidth > 0 ? `${ponts.measuredMolarWidth.toFixed(1)} mm` : '—'}
                </div>
                <p className="text-xs text-slate-500 mt-1 leading-snug">
                  Auto-calculated from teeth 16, 17, 26, 27 in the grid above
                </p>
              </div>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm">
              ...
              {ponts.calculatedMPV && (
                <p className="text-xs text-slate-600 font-medium mt-1">
                  Sum of Incisors (SI): {ponts.sumOfIncisors}mm | Calculated MPV: {ponts.calculatedMPV.toFixed(1)}mm | MMV: {ponts.calculatedMMV?.toFixed(1)}mm
                </p>
              )}
            </div>
          </div>
