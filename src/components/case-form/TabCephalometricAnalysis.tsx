...
import { SnFhCorrectionMatrixPanel } from './SnFhCorrectionMatrixPanel';
import { ComprehensiveCephAnalysis } from './ComprehensiveCephAnalysis';
import { ChevronLeft, ChevronRight, Activity, AlertTriangle, CheckCircle2, Sparkles, RefreshCw, Compass } from 'lucide-react';
import { SnFhCorrectionAnalysisData } from '../../types';

export const CEPH_PAGES = [
  { id: 'downs', label: 'Downs Analysis' },
  { id: 'steiners', label: "Steiner's Analysis" },
  { id: 'snFhCorrection', label: 'SN-FH Cranial Base Correction' },
...
interface TabCephalometricAnalysisProps {
  ...
  onUpdateSnFhCorrectionAnalysis?: (data: SnFhCorrectionAnalysisData) => void;
...
export const TabCephalometricAnalysis: React.FC<TabCephalometricAnalysisProps> = ({
  ...
  onUpdateSnFhCorrectionAnalysis,
  rickettsAnalysis,
  ...
  const handleAutoCalculateAll = () => {
    if (!cephLandmarkData?.landmarks || Object.keys(cephLandmarkData.landmarks).length === 0) return;
    const numAge = typeof patientAge === 'number' ? patientAge : parseInt(String(patientAge), 10) || 12;
    const scalePx = cephLandmarkData.calibration?.scalePixelsPerMm || 10;

    const generated = autoGenerateAllCephAnalyses(
      ...
      {
        downs: downsAnalysis,
        steiners: steinersAnalysis,
        ricketts: rickettsAnalysis,
        mcnamara: mcnamaraAnalysis,
        schwarzTweed: schwarzTweedAnalysis,
        holdaway: holdawayAnalysis,
        cogs: cogsAnalysis,
        cogsSoftTissue: cogsSoftTissueAnalysis,
        cephDiscrepancy: cephDiscrepancyAnalysis,
        verticalJawDivergence: verticalJawDivergenceAnalysis,
      }
    );

    if (onUpdateDownsAnalysis && generated.downsAnalysis) onUpdateDownsAnalysis(generated.downsAnalysis);
    if (onUpdateSteinersAnalysis && generated.steinersAnalysis) onUpdateSteinersAnalysis(generated.steinersAnalysis);
    if (onUpdateRickettsAnalysis && generated.rickettsAnalysis) onUpdateRickettsAnalysis(generated.rickettsAnalysis);
    if (onUpdateMcnamaraAnalysis && generated.mcnamaraAnalysis) onUpdateMcnamaraAnalysis(generated.mcnamaraAnalysis);
    if (onUpdateSchwarzTweedAnalysis && generated.schwarzTweedAnalysis) onUpdateSchwarzTweedAnalysis(generated.schwarzTweedAnalysis);
    if (onUpdateHoldawayAnalysis && generated.holdawayAnalysis) onUpdateHoldawayAnalysis(generated.holdawayAnalysis);
    if (onUpdateCogsAnalysis && generated.cogsAnalysis) onUpdateCogsAnalysis(generated.cogsAnalysis);
    if (onUpdateCogsSoftTissueAnalysis && generated.cogsSoftTissueAnalysis) onUpdateCogsSoftTissueAnalysis(generated.cogsSoftTissueAnalysis);
    if (onUpdateCephDiscrepancyAnalysis && generated.cephDiscrepancyAnalysis) onUpdateCephDiscrepancyAnalysis(generated.cephDiscrepancyAnalysis);
    if (onUpdateVerticalJawDivergenceAnalysis && generated.verticalJawDivergenceAnalysis) onUpdateVerticalJawDivergenceAnalysis(generated.verticalJawDivergenceAnalysis);
  };

  // Auto-run with debouncing when entering or when landmarks update
  useEffect(() => {
    if (cephLandmarkData?.landmarks && Object.keys(cephLandmarkData.landmarks).length > 0) {
      const timer = setTimeout(() => {
        handleAutoCalculateAll();
      }, 250);
      return () => clearTimeout(timer);
    }
  }, [cephLandmarkData?.landmarks, activeStage]);
  ...
  return (
    <div className="w-full max-w-full box-border overflow-x-hidden space-y-3 pb-6">
      ...
      <div className="space-y-3">
        ...
        <div id="ceph-page-snFhCorrection">
          {openAccordion === 'snFhCorrection' && (
            <SnFhCorrectionMatrixPanel
              data={snFhCorrectionAnalysis}
              onChange={onUpdateSnFhCorrectionAnalysis}
