...
export interface BonwillEquilateralTriangle {
  sideLength: number;       // 100.0 mm reference standard
  altitude: number;         // sideLength * sqrt(3) / 2 = 86.60 mm
  apexA: Point2D;           // Incisal Contact Point (0, 0)
  vertexB: Point2D;         // Left Condyle / Posterior (-50, -86.60)
  vertexC: Point2D;         // Right Condyle / Posterior (50, -86.60)
  leftSide: number;         // 100.0 mm
  rightSide: number;        // 100.0 mm
  base: number;             // 100.0 mm
  angles: { angleA: number; angleB: number; angleC: number }; // 60.0°, 60.0°, 60.0°
}
...
export interface CalculatedHawleyGeometry {
  ...
  // Bonwill Equilateral Triangle (1:1 Exact Physical Dimensions)
  bonwillTriangle: BonwillEquilateralTriangle;
...
export function calculateHawleyGeometry(
  ...
  const bonwillSide = 100.0;
  const bonwillAltitude = bonwillSide * (Math.sqrt(3) / 2.0); // 86.6025 mm
  const bonwillTriangle: BonwillEquilateralTriangle = {
    sideLength: bonwillSide,
    altitude: bonwillAltitude,
    apexA: { x: 0.0, y: 0.0 },
    vertexB: { x: -bonwillSide / 2.0, y: -bonwillAltitude },
    vertexC: { x: bonwillSide / 2.0, y: -bonwillAltitude },
    leftSide: bonwillSide,
    rightSide: bonwillSide,
    base: bonwillSide,
    angles: { angleA: 60.0, angleB: 60.0, angleC: 60.0 },
  };

  // Bounding Box
  let minX = Math.min(-2 * r - 10, -bonwillSide / 2.0 - 10);
  let maxX = Math.max(2 * r + 10, bonwillSide / 2.0 + 10);
  let minY = Math.min(-3 * r - 10, -bonwillAltitude - 10);
  let maxY = 10;
