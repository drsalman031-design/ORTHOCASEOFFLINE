...
const TabCephLandmarkIdentification = lazyWithRetry(
  () => import('./case-form/TabCephLandmarkIdentification'),
  'TabCephLandmarkIdentification'
);
...
const TabDentalVTO = lazyWithRetry(
  () => import('./case-form/dental-vto/TabDentalVTO'),
  'TabDentalVTO'
);
...
const TAB_CONFIG: { id: FormTab; label: string; icon: React.ElementType }[] = [
  ...
  { id: 'dentalVto', label: 'Dental VTO', icon: Target },
...
const TAB_PREFETCH: Partial<Record<FormTab, () => Promise<unknown>>> = {
  ...
  dentalVto: () => import('./case-form/dental-vto/TabDentalVTO'),
...
export const CaseForm: React.FC<CaseFormProps> = ({ initialPatient, onSavePatient, onCancel }) => {
  ...
  const [intraoralPhotos, setIntraoralPhotos] = useState<IntraoralPhotos>(
    initialPatient?.intraoralPhotos || initialPatient?.intraoralSection?.photos || {}
  );
  ...
  const [snFhCorrectionAnalysis, setSnFhCorrectionAnalysis] = useState<SnFhCorrectionAnalysisData | undefined>(
    initialPatient?.radiographyGrowth?.snFhCorrectionAnalysis || initialPatient?.snFhCorrectionAnalysis
  );

  const [cephLandmarkModuleData, setCephLandmarkModuleData] = useState<CephLandmarkModuleData | undefined>(
    initialPatient?.cephLandmarkModuleData
  );
  ...
  const [measuredPremolarWidth, setMeasuredPremolarWidth] = useState<number | ''>(
    initialPatient?.modelAnalysis?.measuredPremolarWidth || 36
  );
  const [measuredMolarWidth, setMeasuredMolarWidth] = useState<number | ''>(
    initialPatient?.modelAnalysis?.measuredMolarWidth || 45
  );
  const [dentalVto, setDentalVto] = useState<import('../types/dentalVto').DentalVTOData | undefined>(
    initialPatient?.dentalVto
  );
  ...
  const buildCurrentPatientRecord = (): PatientRecord => {
    const rawRecord: Partial<PatientRecord> = {
      ...
      investigations: {
        ...
        modelAnalysisSummary: `Arch Available: Max ${maxillaryArchLengthAvailable}mm, Mand ${mandibularArchLengthAvailable}mm`,
  ...
  useEffect(() => {
    ...
    midlineUpper, midlineLower, midlineTogether, intraoralPhotos, opgFindings, lateralCephFindings, iopaFindings, handWristFindings,
  ...
  const getSectionCompletionMap = (): Record<FormTab, boolean> => {
    return {
      ...
      dentalVto: Boolean(dentalVto?.currentStatus || overjetMm !== ''),
      ...
      <form onSubmit={(e) => e.preventDefault()} className="space-y-3">
        ...
        {activeTab === 'intraoral' && (
          <TabIntraoral
            ...
            setIntraoralPhotos={setIntraoralPhotos}
          />
        )}
        ...
        {activeTab === 'modelAnalysis' && (
          <TabModelAnalysis
            ...
            measuredPremolarWidth={measuredPremolarWidth}
            setMeasuredPremolarWidth={setMeasuredPremolarWidth}
            measuredMolarWidth={measuredMolarWidth}
            setMeasuredMolarWidth={setMeasuredMolarWidth}
          />
        )}
        ...
        {activeTab === 'cephalometricAnalysis' && (
          <TabCephalometricAnalysis
            ...
            snFhCorrectionAnalysis={snFhCorrectionAnalysis}
            onUpdateSnFhCorrectionAnalysis={setSnFhCorrectionAnalysis}
        ...
        {activeTab === 'dentalVto' && (
          <TabDentalVTO
            ...
            onUpdatePatient={(updatedPatient) => {
              if (updatedPatient.dentalVto) {
                setDentalVto(updatedPatient.dentalVto);
              }
        ...
        <div className="mt-4 pb-4 border-t border-slate-200/80 pt-3">
