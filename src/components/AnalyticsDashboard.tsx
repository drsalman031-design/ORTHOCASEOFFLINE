...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ patients = [] }) => {
  const [timeframe, setTimeframe] = useState<'MONTHLY' | 'QUARTERLY' | 'YEARLY'>('MONTHLY');

  // Dynamic Malocclusion Distribution Data
  const malocclusionData = React.useMemo(() => {
    ...
    patients.forEach((p) => {
      const diagStr = [
        ...
        p.diagnosisAndPlan?.finalDiagnosis,
        ...
        (p as any).diagnosisPlan?.dentalClass,
      ].filter(Boolean).join(' ');
      ...
      const combined = `${diagStr} ${intraoralStr} ${p.chiefComplaint?.additionalNotes || ''}`;

      const isClass3 = /\b(?:class\s*(?:iii|3)|class-iii|prognathic\s+mandible|reverse\s+overjet)\b/i.test(combined);
      const isDiv1 = /\b(?:div(?:ision)?\s*1|class\s*(?:ii|2)\s*div(?:ision)?\s*1)\b/i.test(combined) || (p.chiefComplaint?.protrudingTeeth && p.extraoralProfile?.profile === 'Convex');
      const isDiv2 = /\b(?:div(?:ision)?\s*2|class\s*(?:ii|2)\s*div(?:ision)?\s*2)\b/i.test(combined);
      const isClass2 = /\b(?:class\s*(?:ii|2)|class-ii|retrognathic\s+mandible)\b/i.test(combined);

      if (/\b(?:open\s*bite)\b/i.test(combined)) openBite++;
      else if (/\b(?:deep\s*bite)\b/i.test(combined)) deepBite++;
      else if (/\b(?:surgical|orthognathic)\b/i.test(combined)) surgery++;
      else if (isClass3) class3++;
      else if (isDiv1) class2Div1++;
      else if (isDiv2) class2Div2++;
      else if (isClass2) class2Other++;
      else class1++;
    });

    const items = [
      { name: 'Class 1', count: class1, color: '#00317e' },
      { name: 'Class 2 Div 1', count: class2Div1, color: '#3f5c9e' },
      { name: 'Class 2 Div 2', count: class2Div2, color: '#98b5fd' },
      { name: 'Class 2', count: class2Other, color: '#0046ad' },
      { name: 'Class 3', count: class3, color: '#ba1a1a' },
      { name: 'Open Bite', count: openBite, color: '#00a082' },
      { name: 'Deep Bite', count: deepBite, color: '#2f3742' },
      { name: 'Orthognathic Surgery', count: surgery, color: '#737784' },
    ];

    return items.filter((item) => item.count > 0 || patients.length === 0);
  }, [patients]);

  const totalCasesCount = patients.length > 0 ? patients.length : 126;
        ...
        <section className="bg-white border border-slate-200/80 rounded-2xl p-5 space-y-4 shadow-2xs">
          <div className="flex justify-between items-center">
            ...
            <div className="bg-blue-50 px-3 py-1 rounded-lg border border-blue-100 text-right">
              <span className="text-xs font-extrabold text-[#00317e]">{totalCasesCount} Total</span>
