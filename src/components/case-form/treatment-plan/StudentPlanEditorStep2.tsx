...
export const StudentPlanEditorStep2: React.FC<StudentPlanEditorStep2Props> = ({
  ...
  // Improve Wording using Local Clinical Decision Engine
  const [isImprovingWording, setIsImprovingWording] = useState(false);
  const handleImproveWording = () => {
    if (isLocked) return;
    setIsImprovingWording(true);
    try {
      const planResult = generateOrthoTreatmentPlan(patient);
      if (planResult) {
        ...
        const updatedPlan: StudentTreatmentPlan = {
          ...
          treatmentObjectives:
            ...
            getPts(planResult.treatmentObjectives, '1. Correct sagittal malocclusion\n2. Align arches and optimize soft tissue aesthetics'),
          phase1AlignmentLeveling:
            plan.phase1AlignmentLeveling?.trim() ||
            getPts(planResult.treatmentSequence, '0.014" NiTi -> 0.018" CuNiTi alignment sequence'),
          phase2MolarSpaceClosure:
            plan.phase2MolarSpaceClosure?.trim() ||
            getPts(planResult.anchoragePlanning, 'Transpalatal Arch (TPA) / TAD anchorage with 0.019" x 0.025" SS working wires.'),
          phase3FinishingDetailing:
            plan.phase3FinishingDetailing?.trim() ||
            'Final 0.016" x 0.022" TMA detailing and vertical settling elastics.',
          retentionPhase:
            plan.retentionPhase?.trim() ||
            'Upper Hawley retainer with labial bow + Lower bonded 3-3 lingual retainer.',
        };
        handlePlanUpdate(updatedPlan);
      }
    } catch (err) {
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-2">
            ...
            <textarea
              ...
              rows={6}
              className={`w-full min-h-[140px] sm:min-h-[160px] p-3.5 text-xs sm:text-sm leading-relaxed rounded-xl border resize-y transition-all focus:outline-none focus:ring-2 ${
                isLocked
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-2">
            ...
            <textarea
              ...
              rows={7}
              className={`w-full min-h-[160px] sm:min-h-[190px] p-3.5 text-xs sm:text-sm leading-relaxed rounded-xl border resize-y transition-all focus:outline-none focus:ring-2 ${
                isLocked
                  ? 'bg-slate-100 text-slate-700 border-slate-200 cursor-not-allowed'
                  : 'bg-slate-50/50 text-slate-900 border-slate-300 focus:bg-white focus:border-teal-500 focus:ring-teal-500/20'
              }`}
