import React, { useState, useEffect, lazy, Suspense, startTransition, useCallback } from 'react';
...
function ComponentErrorFallback({ onRetry }: { onRetry?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center p-8 text-center bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 m-4">
      ...
      <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 mb-1">Section loading delayed</h3>
      <p className="text-sm text-slate-500 dark:text-slate-400 mb-4 max-w-sm">
        A temporary network or bundle delay occurred. Click retry to reload this section cleanly.
      </p>
      <button
        onClick={() => (onRetry ? onRetry() : window.location.reload())}
        className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg text-sm font-medium transition cursor-pointer shadow-md shadow-teal-900/20"
      >
        Retry Section
      </button>
    </div>
    ...
    try {
      ...
      try {
        ...
        const FallbackComponent: React.ComponentType<any> = () => (
          <ComponentErrorFallback onRetry={() => window.location.reload()} />
        );
...
export default function App() {
  ...
  const handleTabChange = useCallback((tab: ActiveTab, filter?: string) => {
  ...
  const handleLoginSuccess = useCallback((userAccount?: UserAccount) => {
  ...
  const handleNavigateToCase = useCallback((patientRecordId: string, _sectionId?: string) => {
    setShowNotificationModal(false);
      ...
      try {
        ...
        if (user) {
          setProfile({
            ...
            academicYear: storedProfile?.academicYear || '2024',
  ...
  // Handlers
  const handleSavePatient = useCallback(async (patient: PatientRecord) => {
  ...
  const handleSaveProfile = useCallback(async (newProfile: StudentProfile) => {
  ...
  const handleStartNewCase = useCallback(() => {
    prefetchCaseForm();
    setEditingPatient(null);
    startTransition(() => handleTabChange('form'));
  }, [handleTabChange]);

  const handleEditPatient = useCallback((patient: PatientRecord) => {
    prefetchCaseForm();
    setSelectedPatientModal(null);
