...
interface PatientCardProps {
  ...
  onGeneratePDF: (patient: PatientRecord) => void;
  onToggleArchive: (patientId: string) => void;
  onRequestDelete?: (patient: PatientRecord) => void;
}
...
const PatientCard: React.FC<PatientCardProps> = React.memo(({
  ...
  const sna =
    p.radiographyGrowth?.steinersAnalysis?.parameters?.sna?.pre ??
    (p.radiographyGrowth?.steinersAnalysis as any)?.sna?.pre ??
    p.cephAnalysis?.SNA ??
    82;
  const snb =
    p.radiographyGrowth?.steinersAnalysis?.parameters?.snb?.pre ??
    (p.radiographyGrowth?.steinersAnalysis as any)?.snb?.pre ??
    p.cephAnalysis?.SNB ??
    80;
  const anb =
  ...
  const molarRel =
    ...
    (p.intraoralExam as any)?.molarRight ||
    (p.intraoralExam as any)?.molarClassRight ||
    'Class I';
  const diagnosis =
    ...
    (p.diagnosisAndPlan?.dentalClassification
      ? `${p.diagnosisAndPlan.dentalClassification} (${p.diagnosisAndPlan.skeletalClassification || 'Skeletal Class I'})`
      : 'Orthodontic case assessment in progress');
  ...
  return (
    ...
    >
      <div className="p-4 sm:p-5 space-y-3.5">
        ...
        <div className="flex justify-between items-start gap-3">
          ...
          <div className="shrink-0">
            <span className="bg-blue-50 text-blue-700 border border-blue-200/80 px-2.5 py-1 rounded-md text-xs font-bold flex items-center gap-1">
      ...
      {/* Footer Actions */}
      <div className="px-4 py-3 bg-slate-50 border-t border-slate-100 flex flex-wrap justify-between items-center gap-2 text-xs">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          {onRequestDelete && (
            <button
              ...
              className="bg-white border border-rose-200 text-rose-600 hover:bg-rose-50 px-2.5 py-1.5 rounded-lg flex items-center gap-1 font-bold text-xs cursor-pointer transition-all shadow-2xs"
              title="Delete Case"
            >
          ...
          <button
            ...
            className="bg-white border border-slate-200 text-slate-700 px-2.5 sm:px-3 py-1.5 rounded-lg flex items-center gap-1.5 hover:bg-slate-100 transition-all active:scale-95 font-bold text-xs cursor-pointer shadow-2xs"
          >
          ...
          <button
            ...
            className="bg-[#00317e] text-white px-3 sm:px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 hover:bg-blue-900 transition-all active:scale-95 font-bold text-xs shadow-2xs cursor-pointer"
          >
            <span>View Case</span>
...
export const PatientList: React.FC<PatientListProps> = React.memo(({
  ...
  }, [patients, activeFilter, debouncedSearch]);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-5 pb-8 font-sans box-border min-w-0">
      ...
      <div className="flex gap-2 overflow-x-auto no-scrollbar [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden pb-1 w-full min-w-0">
        ...
        >
          Active Cases ({patients.filter((p) => !p.archived).length})
        </button>
        ...
        <div className="space-y-4">
          {filteredPatients.map((p) => (
            <PatientCard
              ...
              onRequestDelete={(patient) => setPatientToDelete(patient)}
            />
          ))}
        </div>
      )}
