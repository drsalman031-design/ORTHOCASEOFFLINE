...
const FIELD =
  'w-full min-h-9.5 px-3 py-1.5 border border-slate-300 rounded-xl text-sm font-medium bg-white text-slate-900 focus:ring-2 focus:ring-teal-500/20 focus:border-teal-600 cursor-pointer';
const LABEL = 'block text-slate-900 font-bold text-xs mb-1';
const CARD = 'bg-white border border-slate-200 rounded-xl p-3 shadow-xs space-y-3';
const SECTION_TITLE =
  'text-sm font-bold text-slate-900 uppercase tracking-wide flex items-center gap-2 border-b border-slate-200 pb-1.5';
const SUBHEAD = 'text-[11px] font-bold text-slate-500 uppercase tracking-wider';
  ...
  return (
    <div>
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between gap-2 min-h-9.5 px-3 py-1.5 bg-white border border-slate-300 rounded-xl text-sm font-medium text-slate-800 active:bg-slate-50 cursor-pointer"
        aria-expanded={open}
      >
      ...
      {open && (
        <div className="mt-1 rounded-xl border border-slate-200 bg-white shadow-md max-h-52 overflow-y-auto no-scrollbar">
          {options.map((opt) => {
            const s = String(opt);
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 text-left text-sm font-medium border-b border-slate-100 last:border-b-0 cursor-pointer ${
...
function MultiSelectField({
  ...
  return (
    <div>
      ...
      {open && (
        <div
          className="mt-1 rounded-xl border border-slate-200 bg-white shadow-md max-h-52 overflow-y-auto no-scrollbar"
          role="listbox"
  ...
  return (
    <div className="space-y-3 pb-2">
      ...
      <section className={CARD}>
        ...
        <div className="space-y-2.5">
          ...
          <div>
            ...
            />
            <p className="mt-0.5 text-[11px] text-slate-500">Auto-filled for new cases — edit if needed</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => props.setAge(e.target.value === '' ? '' : Number(e.target.value))}
              placeholder="e.g. 14"
              className={FIELD}
            />
          </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['Female', 'Male', 'Other'] as Gender[]).map((g) => (
                <button
                  ...
                  className={`py-1.5 px-3 rounded-xl border text-sm font-semibold transition-all cursor-pointer ${
                    props.gender === g
          ...
          <div>
            ...
            <textarea
              ...
              className={`${FIELD} min-h-[3.75rem] py-1.5 leading-snug resize-y`}
      ...
      <section className={CARD}>
        ...
        <div className="space-y-2.5">
          <MultiSelectField
            label="Primary Complaints (Presets)"
            value={ccPrimaryValue}
            onChange={setCcPrimaryValue}
            options={CC_PRIMARY_OPTIONS}
            placeholder="Tap to select standard complaints…"
          />

          <TextFieldWithSuggestions
            label="Primary Complaint (Manual Entry / Custom)"
            value={props.ccOtherText}
            onChange={props.setCcOtherText}
            suggestions={CC_OTHER_OPTIONS.filter((s) => s !== 'None')}
            placeholder="Type custom complaint manually or select a suggestion…"
          />
