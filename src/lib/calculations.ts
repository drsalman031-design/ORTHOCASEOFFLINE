...
/** Maxillary teeth summed for Carey's total tooth material (15 to 25). */
export const CAREY_MAXILLARY_TEETH = [
  '15', '14', '13', '12', '11', '21', '22', '23', '24', '25',
] as const;

export function sumCareyMaxillaryToothMaterial(toothWidths: Record<string, number | ''>): number {
  return CAREY_MAXILLARY_TEETH.reduce((sum, tooth) => sum + getToothWidthMm(toothWidths, tooth), 0);
}

export function calculateCarey(
  toothWidths: Record<string, number | ''>,
  archLengthAvailable: number | ''
): CareyResult {
  const totalToothMaterial = sumCareyMaxillaryToothMaterial(toothWidths);

  if (typeof archLengthAvailable !== 'number' || isNaN(archLengthAvailable) || totalToothMaterial === 0) {
    return {
      totalToothMaterial,
      discrepancy: null,
  ...
  const discrepancy = archLengthAvailable - totalToothMaterial;

  if (Math.abs(discrepancy) < 0.2) {
    return {
      totalToothMaterial,
      discrepancy,
      inference: 'Normal Arch Perimeter (Perfect Match)',
      badgeColor: 'green',
    };
    ...
    return {
      totalToothMaterial,
      discrepancy,
      inference: `Arch Length Deficiency of ${deficit.toFixed(1)} mm (Crowding / Extraction or Expansion indicated)`,
      badgeColor: 'red',
    };
  } else {
    return {
      totalToothMaterial,
      discrepancy,
      inference: `Arch Length Excess of ${discrepancy.toFixed(1)} mm (Spacing)`,
      badgeColor: 'amber',
    };
  }
}

/** Maxillary premolars summed for Pont's measured 4-4 width. */
export const PONTS_PM_44_TEETH = ['14', '15', '24', '25'] as const;

/** Maxillary molars summed for Pont's measured 6-6 width. */
...
export function calculatePonts(toothWidths: Record<string, number | ''>): PontsResult {
  const sumOfIncisors =
  ...
  const measuredPremolarWidth = sumPontsPremolarWidth44(toothWidths);
  const measuredMolarWidth = sumPontsMolarWidth66(toothWidths);
  ...
  let inference = `Calculated MPV: ${calculatedMPV.toFixed(1)}mm | MMV: ${calculatedMMV.toFixed(1)}mm`;
  ...
  if (premolarExpansionNeeded !== null || molarExpansionNeeded !== null) {
    const pmText =
      premolarExpansionNeeded !== null
        ? `PM Exp: ${premolarExpansionNeeded > 0 ? premolarExpansionNeeded.toFixed(1) + 'mm needed' : 'Sufficient'}`
        : '';
    const mText =
      molarExpansionNeeded !== null
        ? `Molar Exp: ${molarExpansionNeeded > 0 ? molarExpansionNeeded.toFixed(1) + 'mm needed' : 'Sufficient'}`
        : '';
    inference = `Pont Analysis: ${[pmText, mText].filter(Boolean).join(' | ')}`;
  }
...
export function calculateAshleyHowe(
  ...
  if (
    ...
    return {
      pmbaRatio: null,
      inference: 'Enter Premolar Basal Arch Width and Total Tooth Material',
      badgeColor: 'amber',
    };
  }
  ...
  if (pmbaRatio < 37) {
    return {
      pmbaRatio,
      inference: `PMBA W% = ${pmbaRatio.toFixed(1)}% (< 37%): Extraction Indicated (Basal arch cannot accommodate tooth material)`,
      badgeColor: 'red',
    };
  } else if (pmbaRatio >= 37 && pmbaRatio <= 44) {
    return {
      pmbaRatio,
      inference: `PMBA W% = ${pmbaRatio.toFixed(1)}% (37%-44%): Borderline Case / Expansion possible`,
      badgeColor: 'amber',
    };
  } else {
    return {
      pmbaRatio,
      inference: `PMBA W% = ${pmbaRatio.toFixed(1)}% (> 44%): Non-extraction treatment indicated`,
      badgeColor: 'green',
    };
  }
}
