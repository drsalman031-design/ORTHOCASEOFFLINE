...
import {
  generateGeminiMentorData,
  askGeminiMentorQuestion,
  ChatMessage,
} from '../../lib/geminiMentorService';
import {
  MentorModuleData,
...
export const TabAiMentor: React.FC<TabAiMentorProps> = ({ patient, profile }) => {
  ...
  const handleGenerateMentorData = async () => {
    setLoading(true);
    try {
      const data = await generateGeminiMentorData(patient);
      setMentorData(data);
    } catch (err) {
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      const mentorReplyText = await askGeminiMentorQuestion(patient, query, chatMessages);
